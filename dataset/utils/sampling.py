...
def client_iid_indices(dataset, num_clients):
    num_items = int(len(dataset) / num_clients)
    list_client2indices, all_idx = [], [i for i in range(len(dataset))]
    for i in range(num_clients):
        list_client2indices.append(list(set(np.random.choice(all_idx, num_items, replace=False))))
        all_idx = list(set(all_idx) - set(list_client2indices[i]))
    return list_client2indices


def clients_non_iid_indices(list_label2indices: list, num_classes: int, num_clients: int, non_iid_alpha: float,
                            seed=None):
    indices2targets = []
